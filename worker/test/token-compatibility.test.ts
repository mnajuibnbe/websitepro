...
test('rejects an expired token and a token signed with another secret', async () => {
  ...
  await assert.rejects(() => verifyStreamToken(expired, secret), /expired/i);
  const wrongSignature = jwt.sign({ fileId: 'drive-file-123', resourceType: 'video' }, 'another-secret', { expiresIn: '2h' });
  await assert.rejects(() => verifyStreamToken(wrongSignature, secret), /signature/i);
});

test('preserves the video resource-type scope', async () => {
  const documentToken = jwt.sign({ fileId: 'drive-file-123', resourceType: 'pdf' }, secret, { expiresIn: '2h' });
  await assert.rejects(() => verifyStreamToken(documentToken, secret), /video streaming/i);
});
