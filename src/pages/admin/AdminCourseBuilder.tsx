...
export function AdminCourseBuilder() {
  ...
  const fetchCourseCurriculum = useCallback(async () => {
    ...
    try {
      ...
      setErrorMessage(err.message || 'Unable to load the Course Builder.');
    } finally {
      setIsLoading(false);
    }
  }, [courseId]);
  ...
  const handleAddSection = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast('error', err.message || 'Unable to add this item. Please try again.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleUpdateSectionTitle = async (sectionId: string) => {
    ...
    try {
      ...
      addToast('error', 'Unable to update this item. Please try again.');
    }
  };
  ...
  const handleDuplicateLesson = async (lessonId: string) => {
    try {
      ...
      addToast('error', 'Unable to update the lesson. Please try again.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleTogglePublish = async () => {
    ...
    try {
      ...
      addToast('error', 'Unable to update the publishing status. Please try again.');
    }
  };
  ...
  const handleSaveTabSettings = async (tabName: 'seo' | 'pricing') => {
    ...
    try {
      ...
      addToast('error', 'Unable to save the course settings. Please try again.');
    } finally {
      setIsSaving(false);
    }
  };
