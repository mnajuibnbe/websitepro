  ...
  return <div className="min-h-screen bg-primary-50">
    ...
    <main className="pb-20 pt-20 lg:pl-72 lg:pt-8"><PageContainer><div className="mx-auto max-w-5xl">
      ...
      {loading ? <Loader2 className="mx-auto h-8 w-8 animate-spin" /> : loadError ? <div role="alert" className="rounded-2xl border border-danger-200 bg-danger-50 p-8 text-center"><p className="font-bold text-danger-800">Review queue unavailable</p><button type="button" onClick={() => void load()} className="mt-4 rounded-xl bg-primary-900 px-5 py-3 font-bold text-white">Try again</button></div> : rows.length === 0 ? <div className="rounded-2xl border border-dashed bg-white p-12 text-center"><p className="font-bold">No submitted courses are waiting for review.</p><p className="mt-2 text-sm text-primary-500">Draft courses remain with their instructor until they select Submit for review.</p></div> : <div className="space-y-4">{rows.map(row => <article key={row.course_id} className="rounded-2xl border bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => void beginDecision(row, 'approved')} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-success-600 px-4 font-bold text-white"><CheckCircle2 className="h-4 w-4" />Approve and publish</button><button type="button" onClick={() => void beginDecision(row, 'changes_requested')} className="inline-flex min-h-11 items-center gap-2 rounded-xl border px-4 font-bold"><RotateCcw className="h-4 w-4" />Request changes</button><button type="button" onClick={() => void beginDecision(row, 'rejected')} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-danger-200 px-4 font-bold text-danger-700"><XCircle className="h-4 w-4" />Reject</button></div>
      </article>)}</div>}
