...
export function AdminCourseEdit() {
  ...
  const loadCourse = useCallback(async () => {
    if (!courseId) {
      setErrorMessage('Invalid course link.');
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!course) {
        setErrorMessage('Unable to load the course. Please try again.');
        setIsLoading(false);
        return;
      }
      ...
      setErrorMessage(err.message || 'Unable to load the course. Please try again.');
    } finally {
      setIsLoading(false);
    }
  }, [courseId]);
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-primary-600 text-xs sm:text-sm">
                  Update the course information and publishing settings.
                </p>
              </div>
            </div>
          ...
          {isLoading ? (
            <div className="bg-white border border-primary-200 rounded-2xl p-12 text-center shadow-xs">
              ...
              <p className="text-primary-700 font-bold text-sm">Loading course...</p>
            </div>
          ) : errorMessage ? (
            /* Error State */
