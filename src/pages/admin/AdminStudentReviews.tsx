...
import { CheckCircle2, Loader2, MessageSquareText, Star, XCircle } from 'lucide-react';
...
export function AdminStudentReviews() {
  ...
  const loadReviews = useCallback(async () => {
    setLoading(true);
    setMessage('');
    const [{ data: reviewData, error: reviewError }, { data: courseData, error: courseError }] = await Promise.all([
      supabase.rpc('admin_list_pending_course_reviews', { p_course_id: courseId || null }),
      supabase.from('courses').select('id,title').order('title'),
    ]);
    if (reviewError || courseError) setMessage(reviewError?.message || courseError?.message || 'Pending reviews could not be loaded.');
    else {
