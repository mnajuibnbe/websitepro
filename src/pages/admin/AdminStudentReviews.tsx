...
interface LegacyEdit { reviewer_name: string; quote: string; title: string; }
const EMPTY_LEGACY_DRAFT: LegacyEdit = { reviewer_name: '', quote: '', title: 'Tutiba Student' };
...
export function AdminStudentReviews() {
  ...
  const loadReviews = useCallback(async () => {
    ...
    if (reviewError || courseError || legacyError) setMessage(reviewError?.message || courseError?.message || legacyError?.message || 'Reviews could not be loaded.');
      ...
      setLegacyEdits(Object.fromEntries(legacy.map(item => [item.id, { reviewer_name: item.reviewer_name, quote: item.quote, title: item.title }])));
    }
    setLoading(false);
  }, [courseId]);
  ...
  const saveLegacyTestimonial = async (testimonial: LegacyTestimonial) => {
    ...
    const { error } = await supabase.from('legacy_testimonials').update({ reviewer_name, quote, title }).eq('id', testimonial.id);
    setSavingLegacyId(null);
    if (error) setMessage(error.message); else { setMessage(`Updated ${reviewer_name}'s testimonial.`); setLegacyTestimonials(current => current.map(item => item.id === testimonial.id ? { ...item, reviewer_name, quote, title } : item)); }
  };
  ...
  const createLegacyTestimonial = async (event: FormEvent) => {
    ...
    const { error } = await supabase.from('legacy_testimonials').insert({ reviewer_name, quote, title, display_order: nextOrder });
