    ...
    <main id="main-content" className="min-w-0 flex-1 p-4 pb-20 sm:p-8">
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          <div className="flex w-full items-start justify-between gap-3 lg:w-auto lg:items-center lg:justify-start"><button type="button" aria-label="Open navigation" onClick={() => setIsSidebarOpen(true)} className="order-2 inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl border border-primary-200 bg-white lg:order-1 lg:hidden"><Menu className="h-5 w-5" /></button><div className="order-1 lg:order-2"><h1 className="text-2xl font-bold text-primary-900 sm:text-3xl">My authored courses</h1><p className="mt-1 text-sm text-primary-600">Create, edit, and track courses through review and publication.</p></div></div>
          <Link to="/instructor/courses/new" className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-accent-600 px-5 font-bold text-white transition-colors hover:bg-accent-700 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:ring-offset-2 active:bg-accent-800"><Plus className="h-5 w-5" />Create course</Link>
        </div>
