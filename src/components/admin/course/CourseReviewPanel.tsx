...
export function CourseReviewPanel({ courseId }: { courseId: string }) {
  ...
  return <div className="space-y-5">
    ...
    {data.status === 'archived' && <div role="alert" className="rounded-xl border border-warning-200 bg-warning-50 p-4"><p className="font-bold">This course is archived</p><p className="mt-1 text-sm text-primary-700">Restore it to draft from Course details before trying to submit it for review.</p><Link to={`/admin/courses/${courseId}/edit`} className="mt-3 inline-flex min-h-11 items-center rounded-lg border border-warning-200 px-4 font-bold">Open course details</Link></div>}
    {data.reviewer_notes && <div className="rounded-xl border border-warning-200 bg-warning-50 p-4"><p className="font-bold">Reviewer feedback</p><p className="mt-1 whitespace-pre-wrap text-sm">{data.reviewer_notes}</p></div>}
    ...
    <div className="grid gap-3 sm:grid-cols-2">{data.checks.map((check) => {
      ...
      return <div key={check.key} className={`min-h-14 rounded-xl border p-4 ${check.complete ? 'border-success-200 bg-success-100' : 'border-warning-200 bg-warning-50'}`}>
        <div className="flex items-start gap-3">{check.complete ? <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-success-700" aria-hidden="true" /> : <AlertCircle className="mt-0.5 h-5 w-5 shrink-0 text-warning-700" aria-hidden="true" />}<div className="min-w-0 flex-1"><p className="font-bold">{check.label}</p>{!check.complete && check.detail && <p className="mt-1 text-sm text-primary-700">{check.detail}</p>}{!check.complete && check.items?.map((item) => <div key={item.id || item.label} className="mt-2 rounded-lg border border-warning-200 bg-white/60 p-2 text-xs text-primary-700"><p><span className="font-bold">{item.label}</span>{item.detail ? ` — ${item.detail}` : ''}</p>{item.id && <Link to={`/admin/courses/${courseId}/lessons/${item.id}/edit?tab=content`} className="mt-1 inline-flex min-h-11 items-center gap-1 font-bold text-warning-800 underline underline-offset-4 hover:text-warning-900">Open lesson<ExternalLink className="h-3.5 w-3.5" /></Link>}</div>)}{!check.complete && <Link to={readinessTargetHref(courseId, target)} className="mt-2 inline-flex min-h-11 items-center gap-1 text-sm font-bold text-warning-800 underline underline-offset-4 hover:text-warning-900">Resolve this requirement<ExternalLink className="h-3.5 w-3.5" /></Link>}</div></div>
      </div>;
    })}</div>
