...
export function CourseEditorGuide({ steps }: { steps: CourseEditorStep[] }) {
  ...
  return <nav aria-label="Course setup progress" className="mb-8 rounded-2xl border border-primary-200 bg-white p-4 shadow-2xs sm:p-5">
    ...
    <ol className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{steps.map((step, index) => <li key={step.id}><a href={`#${step.id}`} className="flex min-h-16 items-start gap-2 rounded-xl border border-primary-100 p-3 text-left hover:border-amber-300 hover:bg-amber-50/40 focus:ring-2 focus:ring-amber-500"><span className={`mt-0.5 flex h-6 w-6 flex-none items-center justify-center rounded-full text-xs font-bold ${step.complete ? 'bg-success-100 text-success-700' : 'bg-primary-100 text-primary-500'}`}>{step.complete ? <Check className="h-3.5 w-3.5" /> : <Circle className="h-3.5 w-3.5" />}</span><span><span className="block text-sm font-bold text-primary-900">{index + 1}. {step.label}</span><span className="mt-0.5 block text-xs leading-snug text-primary-500">{step.description}</span></span></a></li>)}</ol>
  </nav>;
}
