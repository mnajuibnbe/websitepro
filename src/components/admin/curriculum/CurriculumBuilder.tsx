...
export function CurriculumBuilder({ courseId }: CurriculumBuilderProps) {
  ...
  const loadCurriculum = useCallback(async () => {
    ...
    try {
      ...
      setError('Unable to load the course curriculum. Please try again.');
    } finally {
      setLoading(false);
    }
  }, [courseId]);
  ...
  const handleCreateSection = async () => {
    ...
    try {
      ...
      setError('Unable to create the section. Please try again.');
    }
  };
  ...
  const handleDuplicateSection = async (sectionId: string) => {
    try {
      ...
      setError(err?.message || 'Unable to save the section.');
    }
  };
  ...
  const confirmDeleteSection = async (
    ...
    try {
      ...
      setError(err?.message || 'Unable to delete the section.');
    }
  };
  ...
  const handleDuplicateItem = async (itemId: string) => {
    try {
      ...
      setError('Select a valid lesson before continuing.');
    }
  };
  ...
  const handleTogglePublishItem = async (itemId: string, currentStatus: boolean) => {
    try {
      ...
      setError('Unable to update the curriculum. Please try again.');
    }
  };
  ...
  const handleBulkPublish = async (isPublished: boolean) => {
    try {
      ...
      setError(err?.message || 'Unable to update the curriculum.');
    }
  };
  ...
  if (loading) {
    return (
      <div className="p-12 text-center bg-white rounded-2xl border border-primary-200/80 shadow-2xs space-y-4" dir="ltr">
        ...
        <p className="text-sm font-bold text-primary-700">Loading course...</p>
      </div>
    );
  }
