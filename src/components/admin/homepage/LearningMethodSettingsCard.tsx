...
export function LearningMethodSettingsCard() {
  ...
  useEffect(() => {
    ...
    supabase.from('homepage_learning_method_settings').select('eyebrow_text,heading,subtext,curriculum').eq('id', 1).single()
      .then(({ data, error }) => {
        if (!active) return;
        if (error) { setLoadFailed(true); setMessage({ kind: 'error', text: 'Audience section settings could not be loaded. Reload the page before making changes.' }); }
  ...
  const save = async (event: FormEvent) => {
    ...
    setIsSaving(false);
    setMessage(error ? { kind: 'error', text: error.message } : { kind: 'success', text: 'Audience section updated.' });
  };
  ...
  return (
    <form onSubmit={save} className="rounded-2xl border border-primary-200 bg-white p-6 shadow-sm md:p-8">
      <div className="mb-6 flex items-center gap-3 border-b border-primary-100 pb-5">
        ...
        <div><h2 className="font-bold text-primary-900">Audience section ("Who it's for")</h2><p className="text-sm text-primary-500">The three persona cards shown on the homepage, right below the stats band.</p></div>
      </div>
      ...
      {isLoading ? <div role="status" className="flex min-h-44 items-center justify-center text-primary-500"><Loader2 className="h-7 w-7 animate-spin" /><span className="sr-only">Loading settings</span></div> : (
        <div className="space-y-4">
          ...
          <label className="block text-sm font-bold text-primary-800">Subtext (not currently shown on the homepage)<textarea value={values.subtext} onChange={e => setValues(c => ({ ...c, subtext: e.target.value }))} rows={2} maxLength={600} disabled={loadFailed} className="mt-2 w-full rounded-xl border border-primary-200 bg-white p-3 font-normal disabled:opacity-60" /></label>

          <div>
            <p className="mb-3 text-sm font-bold text-primary-800">Audience personas</p>
            <IconListFieldEditor items={values.curriculum} onChange={curriculum => setValues(c => ({ ...c, curriculum }))} itemLabel="persona" />
          </div>
        </div>
      )}
      ...
      <div className="mt-7 flex justify-end"><Button type="submit" disabled={isLoading || isSaving || loadFailed} isLoading={isSaving} icon={<Save className="h-4 w-4" />}>Save audience section</Button></div>
    </form>
  );
}
