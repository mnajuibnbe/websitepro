...
export function CourseGrid({ filters, onFiltersChange, onResultCountChange }: CourseGridProps) {
  ...
  const appliedFilters = [
    ...
    ...filters.durations.map(value => ({ label: ({ short: 'Learn More 5 Learn More', medium: '5 - 20 Hour', long: 'Learn More 20 Hour' } as Record<string, string>)[value], clear: () => onFiltersChange({ ...filters, durations: filters.durations.filter(item => item !== value) }) })),
    ...
    <div className="flex-grow">
      {appliedFilters.length > 0 && (
        <div className="flex flex-wrap items-center gap-3 mb-8">
          <span className="text-sm font-bold text-primary-900 ml-2">Learn More:</span>
          ...
          <button type="button" onClick={() => onFiltersChange(EMPTY_CATALOG_FILTERS)} className="text-sm font-bold text-accent-600">Learn More</button>
        </div>
      )}
