...
function SpeedMenu({ rate, onChange }: SpeedMenuProps) {
  ...
  return <div ref={containerRef} className="relative">
    <button ref={buttonRef} type="button" aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen(value => !value)} className="flex min-h-11 min-w-11 items-center justify-center rounded-lg border border-white/10 bg-primary-900/90 px-2 text-xs font-semibold text-primary-100 hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-accent-300/60">{rate}x</button>
...
export function VideoPlayer({ src, poster, title, onEnded, onTimeUpdate, autoPlay = false, fill = false, controls = 'full', startAt, resumePlaying = false, resumeSkipThresholdSeconds, onPlaybackStateChange, onPlaybackError, reconnecting = false }: VideoPlayerProps) {
  ...
  return <div ref={frameRef} className={`group relative w-full overflow-hidden bg-black shadow-lg ${fill ? 'h-full' : 'aspect-video rounded-xl'}`}>
    ...
    {controls === 'playback-only' ? <div className="absolute inset-x-0 bottom-0 z-20 flex justify-center bg-gradient-to-t from-primary-900 via-primary-900/70 to-transparent px-3 pb-3 pt-10 text-white">
      <button type="button" onClick={() => void togglePlayback()} aria-label={playing ? 'Pause video' : 'Play video'} className="flex h-11 min-w-11 items-center justify-center rounded-full bg-primary-900/80 px-4 shadow-lg backdrop-blur hover:bg-primary-900 focus:outline-none focus:ring-4 focus:ring-accent-300/60">{playing ? <Pause className="h-5 w-5 fill-current" /> : <Play className="h-5 w-5 fill-current ms-0.5" />}</button>
    </div> : <div className="absolute inset-x-0 bottom-0 z-20 bg-gradient-to-t from-primary-900 via-primary-900/85 to-transparent px-3 pb-2 pt-6 text-white md:px-5 md:pb-3 md:pt-8">
      ...
      <div className="mt-1.5 flex items-center gap-1 sm:gap-2">
        <button type="button" onClick={() => skip(-10)} aria-label="Rewind 10 seconds" className="relative flex min-h-11 min-w-11 items-center justify-center rounded-lg hover:bg-white/10"><RotateCcw className="h-4 w-4" /><span aria-hidden="true" className="pointer-events-none absolute text-[8px] font-bold leading-none">10</span></button>
        <button type="button" onClick={() => void togglePlayback()} aria-label={playing ? 'Pause video' : 'Play video'} className="flex min-h-11 min-w-11 items-center justify-center rounded-lg hover:bg-white/10">{playing ? <Pause className="h-4 w-4 fill-current" /> : <Play className="h-4 w-4 fill-current" />}</button>
        <button type="button" onClick={() => skip(10)} aria-label="Forward 10 seconds" className="relative flex min-h-11 min-w-11 items-center justify-center rounded-lg hover:bg-white/10"><RotateCw className="h-4 w-4" /><span aria-hidden="true" className="pointer-events-none absolute text-[8px] font-bold leading-none">10</span></button>
        <button type="button" onClick={() => { if (!videoRef.current) return; videoRef.current.muted = !videoRef.current.muted; setVolume(videoRef.current.muted ? 0 : videoRef.current.volume); }} aria-label={volume === 0 ? 'Unmute video' : 'Mute video'} className="flex min-h-11 min-w-11 items-center justify-center rounded-lg hover:bg-white/10">{volume === 0 ? <VolumeX className="h-4 w-4" /> : <Volume2 className="h-4 w-4" />}</button>
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          <button type="button" onClick={() => void toggleFullscreen()} aria-label={isFullscreen ? 'Exit fullscreen' : 'Enter fullscreen'} className="flex min-h-11 min-w-11 items-center justify-center rounded-lg hover:bg-white/10">{isFullscreen ? <Minimize className="h-4 w-4" /> : <Maximize className="h-4 w-4" />}</button>
        </div>
      </div>
    </div>}
  </div>;
