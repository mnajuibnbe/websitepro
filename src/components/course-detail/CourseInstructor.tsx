...
export function CourseInstructor({ instructor }: { instructor: PublicInstructorProfile | null }) {
  ...
  return <section className="mb-12 md:mb-16" aria-labelledby="instructor-heading">
    <h2 id="instructor-heading" className="mb-6 text-2xl font-bold text-primary-900 md:text-3xl">Your instructor</h2>
    <div className="rounded-panel border border-primary-200 bg-white p-6 shadow-sm md:p-8">
      <div className="flex flex-col items-center gap-6 text-center md:flex-row md:items-start md:text-left">
        <div className="flex h-32 w-32 flex-none items-center justify-center rounded-full bg-accent-50 text-accent-700" aria-hidden="true">
          <FlaskConical className="h-14 w-14" />
        </div>
        <div className="min-w-0 flex-1">
          <h3 className="text-2xl font-bold text-primary-900">Taught by {instructor.professional_name}</h3>
          <p className="mt-2 leading-relaxed text-primary-600">{instructor.bio}</p>
          <div className="mt-5 flex flex-wrap justify-center gap-2 md:justify-start">{instructor.expertise.map(item => <span key={item} className="rounded-full bg-accent-50 px-3 py-1 text-sm font-bold text-accent-800">{item}</span>)}</div>
          <div className="mt-5 flex items-start gap-3 rounded-xl bg-primary-50 p-4 text-left">
            <Award className="mt-0.5 h-5 w-5 flex-none text-warning-600" />
            <div>
              <p className="font-bold text-primary-900">Professional credentials</p>
              <p className="mt-1 text-sm leading-relaxed text-primary-600">{instructor.credentials}</p>
            </div>
          </div>
        </div>
      </div>
