...
export function CourseMediaLightbox({ open, onClose, courseId, courseTitle, trailerUrl, previewLesson, poster }: { open: boolean; onClose: () => void; courseId: string; courseTitle: string; trailerUrl: string | null; previewLesson: CoursePreviewLesson | null; poster?: string; }) {
  ...
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-primary-950/90 p-4 backdrop-blur-sm" onMouseDown={event => event.target === event.currentTarget && onClose()}><div role="dialog" aria-modal="true" aria-label={title} className="w-full max-w-5xl overflow-hidden rounded-2xl border border-primary-700 bg-primary-950 shadow-2xl"><header className="flex items-center justify-between gap-4 border-b border-primary-800 px-4 py-3 text-white"><div><p className="text-xs font-bold uppercase tracking-eyebrow text-accent-300">{trailer ? 'Course trailer' : 'Free lesson preview'}</p><h2 className="font-bold">{title}</h2></div><button ref={closeRef} type="button" onClick={onClose} aria-label="Close video" className="flex h-11 w-11 items-center justify-center rounded-full hover:bg-white/10"><X className="h-5 w-5" /></button></header><div className="bg-black">{trailer ? provider(trailer) === 'google_drive' ? <SecureStreamProvider asset="course-trailer" courseId={courseId} publicPreview autoPlay title={title} poster={poster} /> : <VideoProviderResolver lessonId={courseId} videoUrl={trailer} provider={provider(trailer)} title={title} poster={poster} publicPreview /> : previewLesson ? <VideoProviderResolver lessonId={previewLesson.lesson_id} videoUrl={previewLesson.video_url} provider={provider(previewLesson.video_url)} title={title} poster={poster} publicPreview /> : null}</div></div></div>;
}
