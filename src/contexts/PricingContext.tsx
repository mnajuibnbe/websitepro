...
const Context = createContext<PricingContextValue>(DEFAULT_PRICING_CONTEXT);
export function PricingProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/pricing/context', { headers: token ? { Authorization: `Bearer ${token}` } : undefined, signal: controller.signal })
      ...
      .then(setValue).catch(error => { if (error.name !== 'AbortError') setValue(DEFAULT_PRICING_CONTEXT); });
