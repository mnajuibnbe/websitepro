...
test('unknown locations use the stable Egypt-first EGP default', () => assert.equal(resolveCoursePrice(course, DEFAULT_PRICING_CONTEXT).formatted, 'EGP 1,500'));
test('international and missing country resolve USD', () => {
  ...
  assert.equal(resolveCoursePrice(course, DEFAULT_PRICING_CONTEXT).currency, 'EGP');
});
