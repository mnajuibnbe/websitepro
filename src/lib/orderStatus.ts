...
export function deriveOrderStatus({ payment_status, enrollment_status, latestSubmissionStatus, latestRejectionReason }: OrderStatusInput): OrderStatusInfo {
  ...
  if (latestSubmissionStatus === 'rejected') {
    return {
      ...
      label: 'Rejected',
      badgeVariant: 'danger',
      description: latestRejectionReason
        ? `We could not verify this payment: ${latestRejectionReason}. You can submit a new payment proof below.`
        : 'We could not verify this payment. Message us on WhatsApp for the reason and how to proceed.',
    };
  }
