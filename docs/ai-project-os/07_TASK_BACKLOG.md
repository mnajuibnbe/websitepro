...
## High Priority
1. **Admin Authorization (RBAC)** (DESIGNED - PENDING IMPLEMENTATION): Implement the RBAC architecture designed in SECURITY-001 (COMPLETED) (`m.najuib.nbe@gmail.com`) in `AdminDashboard.tsx` and `AdminSidebar.tsx`. Replace with role-based checks (e.g., checking `user.role` from auth metadata). Enforce `requiredRole="admin"` properly on `<ProtectedRoute>` inside `App.tsx`.
