...
|---|---|---|
| Published featured course on Home | Pass (logic) | Query requires `published` + `is_featured=true`; unit test covers predicate. |
| Published non-featured Home behavior | Pass (logic) | Excluded by the established featured-section rule. |
...
4. Publish A, set public visibility and featured=true. In an incognito anonymous session confirm A appears on `/` and `/courses`, and `/course/:id` opens.
5. Set featured=false while leaving published/public. Confirm A disappears from Home but remains in `/courses`.
...
8. Create a pending enrollment for S/A. Confirm it remains absent from owned-course lists, course detail reports pending, and `/learn/:courseId` reports pending/denies lessons.
