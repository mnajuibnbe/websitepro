...
const integrityMigration = await readFile(join(migrationDirectory.pathname, '20260730070000_authoring_data_integrity_foundation.sql'), 'utf8').catch(() => '');
...
const autoPublicationMigration = await readFile(join(migrationDirectory.pathname, '20260730080000_review_auto_publication.sql'), 'utf8').catch(() => '');
...
const seoMigration = await readFile(join(migrationDirectory.pathname, '20260730090000_english_and_course_seo.sql'), 'utf8').catch(() => '');
...
console.log(`Course authoring release verification passed: ${expected.length + 3} ordered migrations and 13 critical RPCs.`);
